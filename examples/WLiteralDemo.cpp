...
int main() {
  constexpr auto hi = L"Hello %s!\n"_printf;
  constexpr auto decimal_formatter = L"Decimal: %d\n"_printf;
  constexpr auto lower_hex_formatter = L"hex: %x\n"_printf;
  constexpr auto upper_hex_formatter = L"HEX: %X\n"_printf;
  constexpr auto char_formatter = L"char: '%c'\n"_printf;
  ...
  constexpr auto char_formatted = char_formatter(L'Z');
  constexpr auto hiworld = hi(L"world");
  std::wstring str;
  str << hiworld << decimal_formatted;
  std::wcout.write(str.data(), str.size());
  ...
  constexpr auto base = L"%c%d%x%X%s"_printf;
  constexpr auto derived1 = base + base;
  constexpr auto expected1 = L"%c%d%x%X%s%c%d%x%X%s"_printf;
  ...
  // monoid:
  static_assert(
      std::is_same_v<std::decay_t<decltype(base)>,
                     std::decay_t<decltype(
                         base + philwill::FormattingFunction<wchar_t>())>>,
      "Addition failure (post identity)");
  static_assert(
      std::is_same_v<std::decay_t<decltype(base)>,
                     std::decay_t<decltype(
                         philwill::FormattingFunction<wchar_t>() + base)>>,
      "Addition failure (post identity)");
  static_assert(
      std::is_same_v<decltype((L"a"_printf + L"b"_printf) + L"c"_printf),
                     decltype(L"a"_printf + (L"b"_printf + L"c"_printf))>,
